<main class="flex-1 lg:ml-72 min-h-screen p-6 md:p-10" x-data="signatureCreate()">
    <!-- Header -->
    <header class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
            ...
            <p class="text-slate-500 font-bold mt-1">
                Tambahkan tanda tangan baru untuk Kepala Sekolah
            </p>
        </div>
    </header>

    <!-- Auto-save Indicator -->
    <div x-show="formData.role || formData.name || formData.nip"
    ...
    <div class="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden"
        ...
        x-transition:enter-end="opacity-100 translate-y-0">
        <form wire:submit.prevent="save" class="p-8 md:p-10">
            <div class="max-w-2xl space-y-6">
                ...
                <div>
                    ...
                    <!-- Preview -->
                    @if ($previewUrl)
                        <div class="mt-4 bg-slate-50 rounded-2xl p-6 flex items-center justify-center border-2 border-blue-200"
                            x-transition:enter="transition ease-out duration-300"
                            x-transition:enter-start="opacity-0 scale-95"
                            x-transition:enter-end="opacity-100 scale-100">
                            <img src="{{ $previewUrl }}" alt="Preview tanda tangan" class="max-h-40 max-w-full object-contain">
                        </div>
                    @endif
                        ...
                        <div class="flex gap-3">
                            ...
                            <div>
                                ...
                                <ul class="text-xs font-bold text-blue-800 space-y-1">
                                    ...
                                    <li>• Untuk hasil terbaik, gunakan PNG dengan background transparan</li>
                                </ul>
                            </div>
                        </div>
                    </div>
        ...
        function signatureCreate() {
            return {
                ...
                formData: Alpine.$persist({
                    ...
                    name: '',
                    nip: ''
                }).as('signature_create_form'),
                
                init() {
                    // Sync persisted data with Livewire
                    if (this.formData.role) {
                        @this.set('role', this.formData.role);
                    }
                    if (this.formData.name) {
                        @this.set('name', this.formData.name);
                    }
                    if (this.formData.nip) {
                        @this.set('nip', this.formData.nip);
                    }
                ...
                clearFormData() {
                    ...
                    localStorage.removeItem('signature_create_form');
                }
            }
        }
    </script>
