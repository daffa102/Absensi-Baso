    ...
    <header class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10">
        <div>
            ...
            <p class="text-slate-500 font-bold mt-1">
                Kelola tahun ajaran di sekolah
            </p>
        </div>
        <a href="{{ route('admin.tahun-ajar.create') }}" wire:navigate
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left">
                ...
                <tbody class="divide-y divide-slate-50">
                    @forelse ($tahunAjars as $index => $tahun)
                        <tr class="hover:bg-slate-50/50 transition-colors">
                            <td class="px-8 py-5">
                                <span class="text-sm font-bold text-slate-600">
                                    {{ $tahunAjars->firstItem() + $index }}
                                </span>
                            </td>
                            <td class="px-8 py-5">
                                <div class="flex items-center gap-3">
                                    <div
                                        class="w-10 h-10 bg-purple-100 rounded-xl flex items-center justify-center text-purple-600 font-bold text-sm">
                            ...
                            <td class="px-8 py-5">
                                <button wire:click="toggleAktif({{ $tahun->id }})"
                                    class="inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-[11px] font-black uppercase transition-all {{ $tahun->aktif ? 'bg-green-100 text-green-600 hover:bg-green-200' : 'bg-slate-100 text-slate-600 hover:bg-slate-200' }}">
                                    @if ($tahun->aktif)
                            ...
                            <td class="px-8 py-5">
                                <span class="text-sm font-bold text-slate-600">
                                    {{ $tahun->siswas->count() }} siswa
                                </span>
                            </td>
                            <td class="px-8 py-5">
                                <span class="text-sm font-bold text-slate-600">
                                    {{ $tahun->created_at->format('d M Y') }}
                                </span>
                            </td>
                            <td class="px-8 py-5">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ route('admin.tahun-ajar.edit', $tahun->id) }}" wire:navigate
                        ...
                        <tr>
                            <td colspan="6" class="px-8 py-12 text-center">
                                <div class="flex flex-col items-center gap-3">
                                    ...
                                    <div>
                                        <p class="text-slate-900 font-black text-lg">Tidak ada data tahun ajaran</p>
                                        <p class="text-slate-500 font-bold text-sm mt-1">
                                            @if ($search)
                                                Tidak ditemukan tahun ajaran dengan kata kunci "{{ $search }}"
                                            @else
                                                Mulai tambahkan tahun ajaran baru
                                            @endif
                                        </p>
                                    </div>
                                </div>
